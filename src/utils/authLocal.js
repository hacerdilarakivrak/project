// SHA-256 ile parola hash'leme (Web Crypto)
...
export function saveUsers(users) { write("users", users); }

export function getSession() { return read("session", null); }
export function setSession(s) { write("session", s); }
export function clearSession() { localStorage.removeItem("session"); }

// Basit brute-force kısıtlama: 5 deneme/10 dk
function getAttempts() { return read("loginAttempts", {}); }
function saveAttempts(a) { write("loginAttempts", a); }
export function canAttempt(username) {
  const a = getAttempts()[username] || { count: 0, ts: 0 };
  const now = Date.now();
  const windowMs = 10 * 60 * 1000;
  if (now - a.ts > windowMs) return true; // pencere sıfırlandı
  return a.count < 5;
}
export function recordAttempt(username, ok) {
  ...
  if (!ok) { cur.count += 1; cur.ts = now; }
  else { cur.count = 0; cur.ts = now; } // başarılı olursa sıfırla
  all[username] = cur; saveAttempts(all);
}
...
export async function login(username, password) {
  ...
  if (!u) { recordAttempt(username, false); throw new Error("Kullanıcı bulunamadı."); }
